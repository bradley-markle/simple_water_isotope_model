function [f, P, e_s, r_s, dPdT,ss,ss_l] = pseudo_adiabat_function(T,P0,fraction_i, fraction_l,supersat,sa,sb,sc)
% BRM March 2017
% This funciton calculates a pseudo adiabat. It takes into account a
% mixture of ice and water in the air parcel. The user can specify the
% nature of the super saturation.
%
% Routine for calculating saturated pseudo adiabatic change
...
% We will follow the above, except we are going to use a different set of equations for the
...
%other profiles
e_s = nan(size(T));
r_s = nan(size(T));%g h2o /kg air
r_s_i = nan(size(T));%g h2o /kg air
r_s_l = nan(size(T));%g h2o /kg air
r_s_mixed = nan(size(T));%g h2o /kg air
factor = nan(size(T));
b = nan(size(T));
dPdT=nan(size(T));
...
%OK now the air parcel will be saturated as a function of T
% sa=1;
% sb=0.0054;
% sc=0.0000;

%this is the prescribed version.
S =sa-sb.*T-sc.*(T.^2);
S(S<1)=1;

% ind=supersat==max(supersat);
% SB=1.5;
% % SB=max(supersat);
% S2 = supersat(T>=T(ind));
% linelength = length(T(T<T(ind)));
% S1 = linspace(max(supersat),SB, linelength); 
% S = [S2,S1];
...
r_s_mixed(i) = fraction_l(i).*r_s_l(i) + fraction_i(i).*r_s_i(i);

%now apply the user specified super saturation function
if T(i)>0%T_hi
    factor(i) =  supersat(i)./(r_s_mixed(i)./r_s_i(i));
else
factor(i) = S(i)./(r_s_mixed(i)./r_s_i(i));
end
r_s(i)=factor(i) *r_s_mixed(i);
...
% r_s_mixed(length(T)) = eta * e_s(end) / (P(end) - e_s(end));
r_s_i(end) = eta * e_s_i(end) / (P(end) - e_s_i(end));
r_s_l(end) = eta * e_s_l(end) / (P(end) - e_s_l(end));
r_s_mixed(end) = fraction_l(end).*r_s_l(end) + fraction_i(end).*r_s_i(end);
if T(i)>0
    factor(end) =  supersat(end)./(r_s_mixed(end)./r_s_i(end));
else   
    factor(end) = S(end)./(r_s_mixed(end)./r_s_i(end));
end
r_s(end)=factor(end) *r_s_mixed(end);
b(end) = (1 + (r_s(end)/eta))/(1 + (r_s(end)/c_eff(end)));
dPdT(end) = (P(end)/b(end)) *  (C_pd + ((L_eff(end).^2)*r_s(end)*eta*b(end))/(R_d*(TK(end).^2))) / (R_d*TK(end) + L_eff(end)*r_s(end));
