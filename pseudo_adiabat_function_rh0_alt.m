function [f, P, e_s, r_s, dPdT,ss,rh] = pseudo_adiabat_function_rh0_alt(T,P0,rh0,fraction_i, fraction_l,supersat,sa,sb,sc,RH_in)
% BRM March 2017
...
% ss(T>0)=1;
% f=r_s./(r_s(1)*rh0);
f_test=(r_s.*RH_in)./(r_s(1)*rh0);
f=f_test;
f(f>1)=1;
% rh=(r_s(1)*rh0)./r_s;
rh=(f.*((r_s(1)*rh0)))./r_s;
rh_test=(f_test.*((r_s(1)*rh0)))./r_s;
% rh=rh_test;
rh(f<1)=1;
