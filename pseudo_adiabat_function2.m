function [f, P, e_s, r_s, dPdT,ss] = pseudo_adiabat_function2(T,P0,fraction_i, fraction_l,supersat,P_s)
% BRM March 2017
...
S=polyval(P_s,T);
% S=supersat;
% S = mixed_phased_supersaturation(T,P0,0.5.*ones(size(T)), 0.5.*ones(size(T)));
S(S<1)=1;
...
r_s(i) = fraction_l(i).*r_s_l(i) + fraction_i(i).*r_s_i(i);

%now apply the user specified super saturation function
if T(i)>0%T_hi
    factor(i) =  supersat(i)./(r_s(i)./r_s_i(i));
else
factor(i) = S(i)./(r_s(i)./r_s_i(i));
end
r_s(i)=factor(i) *r_s(i);
...
factor(length(T)) = S(length(T))./(r_s(length(T))./r_s_i(length(T)));
r_s(length(T))=factor(length(T)) *r_s(length(T));
